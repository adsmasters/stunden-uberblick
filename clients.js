(function () {
  ...
  clientModalSave.addEventListener('click', function () {
    ...
    var fields = { name: name, am_budget: am, adv_budget: adv,
                   ...
                   project_end: contractEnd }; // keep project_end in sync for compat
    var promise = editingClientId
      ? window.db.clients.update(editingClientId, fields)
      : window.db.clients.create(name, am, adv, amEmp, advEmp, contractStart, isProject, contractEnd, contractEnd);
  ...
  function typeBadge(c) {
    ...
    var sub = parts.length
      ...
      : '';
    // grey out if contract has ended
    var today = new Date();
